...
import com.daniel.banklist.nonintersect
...
class PermissionListUtil(private var PERMISSION_ASKER : Int){
    ...
    * */
    ...
    fun setAskerCode (code : Int ){
        PERMISSION_ASKER = code
    }
    ...
    fun checkAndAskPermissions(activity : Activity, permissionList : Array<String>, listener : PermissionListAskListener, code : Int = PERMISSION_ASKER) {
        ...
        if (unGrantedPermissionList.isEmpty()) {
            showLogD("All Permission Granted")
            listener.onAllPermissionGranted()
            return
        } else {
            for (grantedPermission in grantList) {
                showLogD("$grantedPermission permission is granted")
    ...
    private fun askPermission(activity : Activity , permission : String ,listener : PermissionListAskListener) {
        ...
        if (activity.shouldShowRequestPermissionRationale(permission)) {
            showLogD("$permission permission is previously denied and asked again")
            ...
            if (isFirstTimeAskingPermission(activity , permission)) {
                ...
                showLogD("$permission permission is needed")
                ...
                requestPermission(activity , permission )
                     ...
                     * */
                showLogD("$permission permission is disabled")
    ...
    fun requestPermission(activity : Activity , permissionList : Array<String> ) {
        if (PERMISSION_ASKER == -1) {
            ...
            ActivityCompat.requestPermissions(
                activity , permissionList ,PERMISSION_ASKER
            )
    }
